...
//				      plat->freq_normal, CPUFREQ_RELATION_H); //orig
				      plat->freq_lemot, CPUFREQ_RELATION_H); //add eve oc
}
...
static inline void cpufreq_bcm_turbo_on(void)
{
	...
	if (bcm_cpufreq_gov_info.cur_policy)
		cpufreq_driver_target(bcm_cpufreq_gov_info.cur_policy,
//				      plat->freq_turbo, CPUFREQ_RELATION_H); //orig
				      plat->freq_makwuz, CPUFREQ_RELATION_H); //add eve oc
}
...
//		pr_info("%s: switching to MAKWUZ mode: %s\n", //add eve oc
			__func__, desc->name);
	...
	unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		 ...
		 */
		if (dvfs_disable == 0)
//			__cpufreq_driver_target(policy, plat->freq_normal, //orig
			__cpufreq_driver_target(policy, plat->freq_lemot, //add eve oc
						CPUFREQ_RELATION_H);
		else
//			__cpufreq_driver_target(policy, plat->freq_turbo, //orig
			__cpufreq_driver_target(policy, plat->freq_makwuz, //add eve oc
						CPUFREQ_RELATION_H);
		break;
