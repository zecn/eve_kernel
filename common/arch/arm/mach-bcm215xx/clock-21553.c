...
//	arm11_freq[0] = FREQ_MHZ(312); //orig
//	arm11_freq[1] = (apps_pll_freq*2)/3; //orig
	//add eve oc
	arm11_freq[0] = FREQ_MHZ(104);
	arm11_freq[1] = FREQ_MHZ(156);
	arm11_freq[2] = FREQ_MHZ(312);
	arm11_freq[3] = FREQ_MHZ(468);
	arm11_freq[4] = FREQ_MHZ(624);
	arm11_freq[5] = (apps_pll_freq*2)/3;
	arm11_freq[6] = (apps_pll_freq*3)/4;
	arm11_freq[7] = apps_pll_freq; 
	//add eve oc
...
{
//	u32 arm11_freq[2]; //orig
	u32 arm11_freq[8]; //add eve oc
	u32 apps_pll_freq = bcm21553_apps_pll_get_rate();
...
//	if(mode < 0xC) //orig
	...
	{
		apps_pll_freq = bcm21553_apps_pll_get_rate();
//		ahb = ((apps_pll_freq/9)/FREQ_MHZ(1))*FREQ_MHZ(1); //orig
		ahb = apps_pll_freq/9; //add eve oc
	}
...
unsigned long bcm21553_ahb_fast_get_rate(struct clk *clk)
{
	...
	{
		apps_pll_freq = bcm21553_apps_pll_get_rate();
//		ahb_fast = (mode <= 8) ? apps_pll_freq/9 : apps_pll_freq/6; //orig
		ahb_fast = apps_pll_freq/6; //add eve oc
	}
//	ahb_fast = (ahb_fast*FREQ_MHZ(1))/FREQ_MHZ(1); //orig
...
//	writel(0x0, ADDR_SYSCFG_CAMARA_INTERFACE_AHB_CLK_EN);	/* disable AHB clock */ //orig
	writel(0x00, ADDR_SYSCFG_CAMARA_INTERFACE_AHB_CLK_EN);	/* disable AHB clock */ //add eve oc
}
...
/*		arm11 = (apps_pll_freq*2)/arm11_div[mode -0xD];
		cp_clk = apps_pll_freq/cp_clk_div[mode -0xD]; */ //orig
		arm11 = (apps_pll_freq*2)/arm11_div[mode -0x0D]; //add eve oc
		cp_clk = apps_pll_freq/cp_clk_div[mode -0x0D]; //add eve oc
		ahb = apps_pll_freq/9;
		ahb_fast = apps_pll_freq/6;
		v3d_clk = apps_pll_freq/4;
	}

//	dsp_mode = readl(ADDR_CLKPWR_CLK_DSP_MODE) & 0x7; //orig
	dsp_mode = readl(ADDR_CLKPWR_CLK_DSP_MODE) & 0x07; //add eve oc
	if(dsp_mode == 0)
	...
	{
		sdram = apps_pll_freq /
//		    clk_sdram_mode_reg_to_appspll_divider[sdram_mode & 0x3]; //orig
		    clk_sdram_mode_reg_to_appspll_divider[sdram_mode & 0x3]; //add eve oc
	}
...
//	pg += sprintf(pg,"SDRAM clk= %uMhz  (%s)\n",sdram, (sdram_mode & 0x4) ? "Sync Mode" : "Async Mode"); //orig
	pg += sprintf(pg,"SDRAM clk= %uMhz  (%s)\n",sdram, (sdram_mode & 0x4) ? "Sync Mode" : "Async Mode"); //add eve oc
