...
static int bcm_avs_drv_probe(struct platform_device *pdev)
{
	...
	/* It is assumed that, at boot up system is in lemot mode */ //add eve oc
	if (pdata->core_lemot_regl && type_info->nm2_lemot_voltage != -1)
	{
		regl = regulator_get(NULL, pdata->core_lemot_regl);
		if(IS_ERR(regl))
		{
			pr_info("%s: Core Lemot mode regulator_get failed\n",
				__func__);
			ret = PTR_ERR(regl);
			goto error;
		}
		regulator_set_voltage(regl, type_info->nm2_lemot_voltage,
				      type_info->nm2_lemot_voltage);
		regulator_put(regl);
	}

	/* It is assumed that, at boot up system is in lelet mode */ //add eve oc
	if (pdata->core_lelet_regl && type_info->nm2_lelet_voltage != -1)
	{
		regl = regulator_get(NULL, pdata->core_lelet_regl);
		if(IS_ERR(regl))
		{
			pr_info("%s: Core Lelet mode regulator_get failed\n",
				__func__);
			ret = PTR_ERR(regl);
			goto error;
		}
		regulator_set_voltage(regl, type_info->nm2_lelet_voltage,
				      type_info->nm2_lelet_voltage);
		regulator_put(regl);
	}

	/* It is assumed that, at boot up system is in normal mode */ //add eve oc
	...
	/* It is assumed that, at boot up system is in kebut mode */ //add eve oc
	if (pdata->core_kebut_regl && type_info->nm2_kebut_voltage != -1)
	{
		regl = regulator_get(NULL, pdata->core_kebut_regl);
		if(IS_ERR(regl))
		{
			pr_info("%s: Core Kebut mode regulator_get failed\n",
				__func__);
			ret = PTR_ERR(regl);
			goto error;
		}
		regulator_set_voltage(regl, type_info->nm2_kebut_voltage,
				      type_info->nm2_kebut_voltage);
		regulator_put(regl);
	}

	/* It is assumed that, at boot up system is in kenceng mode */ //add eve oc
	if (pdata->core_kenceng_regl && type_info->nm2_kenceng_voltage != -1)
	{
		regl = regulator_get(NULL, pdata->core_kenceng_regl);
		if(IS_ERR(regl))
		{
			pr_info("%s: Core Kenceng mode regulator_get failed\n",
				__func__);
			ret = PTR_ERR(regl);
			goto error;
		}
		regulator_set_voltage(regl, type_info->nm2_kenceng_voltage,
				      type_info->nm2_kenceng_voltage);
		regulator_put(regl);
	}
	...
	/* It is assumed that, at boot up system is in ngenos mode */ //add eve oc
	if (pdata->core_ngenos_regl && type_info->nm2_ngenos_voltage != -1)
	{
		regl = regulator_get(NULL, pdata->core_ngenos_regl);
		if(IS_ERR(regl))
		{
			pr_info("%s: Core Ngenos mode regulator_get failed\n",
				__func__);
			ret = PTR_ERR(regl);
			goto error;
		}
		regulator_set_voltage(regl, type_info->nm2_ngenos_voltage,
				      type_info->nm2_ngenos_voltage);
		regulator_put(regl);
	}

	/* It is assumed that, at boot up system is in makwuz mode */ //add eve oc
	if (pdata->core_makwuz_regl && type_info->nm2_makwuz_voltage != -1)
	{
		regl = regulator_get(NULL, pdata->core_makwuz_regl);
		if(IS_ERR(regl))
		{
			pr_info("%s: Core Makwuz mode regulator_get failed\n",
				__func__);
			ret = PTR_ERR(regl);
			goto error;
		}
		regulator_set_voltage(regl, type_info->nm2_makwuz_voltage,
				      type_info->nm2_makwuz_voltage);
		regulator_put(regl);
	}
